...
export default function CartDrawer({ open, onClose, cart, onRemove }) {
  ...
  const handleOrderAll = () => {
    ...
    const items = cart
      .map((i) => `• ${i.name} — Taille: ${i.size} — Couleur: ${i.color} — Qté: ${i.qty} — ${i.price * i.qty} MAD`)
      .join("\n");
      ...
      <aside className={`cart-drawer ${open ? "cart-drawer--open" : ""}`}>
        ...
        <div className="cart-drawer__items">
          {cart.length === 0 ? (
            <div className="cart-drawer__empty">
              ...
              <button onClick={onClose} className="btn btn--outline">Continuer les achats</button>
            </div>
          ) : (
            cart.map((item, idx) => (
              <div key={idx} className="cart-item">
                ...
                <div className="cart-item__info">
                  ...
                  <p className="cart-item__meta">{item.size} · {item.color}</p>
                  <p className="cart-item__price">{item.price * item.qty} MAD</p>
                </div>
                <button
        ...
        {cart.length > 0 && (
          <div className="cart-drawer__footer">
            ...
            <button className="btn btn--whatsapp btn--full" onClick={handleOrderAll}>
