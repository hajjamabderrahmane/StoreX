...
export default function ProductCard({ product, onClick, onAddToCart }) {
  ...
  const handleQuickOrder = (e) => {
    ...
    window.open(buildWhatsAppLink(product.name, size, color), "_blank");
  };
  ...
  return (
    ...
    >
      <div className="product-card__image-wrap">
        ...
        {product.badge && (
          <span
            ...
            style={{ background: BADGE_COLORS[product.badge] || "#111" }}
          >
            {product.badge}
          </span>
        )}
        ...
        <div className={`product-card__overlay ${hovered ? "product-card__overlay--visible" : ""}`}>
          <div className="product-card__sizes">
            {product.sizes.map((s) => (
              <button
                key={s}
                className={`product-card__size ${quickSize === s ? "product-card__size--active" : ""}`}
                onClick={(e) => { e.stopPropagation(); setQuickSize(s); }}
              >
                {s}
              </button>
            ))}
          </div>
          <button className="product-card__whatsapp" onClick={handleQuickOrder}>
      ...
      <div className="product-card__info">
        ...
        <p className="product-card__price">{product.price} MAD</p>
      </div>
    </div>
  );
}